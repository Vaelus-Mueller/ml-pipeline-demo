...
def generate_synthetic_data(n_samples=100, random_seed=42):
    ...
    incomes = ages * 1200.0 + np.random.normal(0, 15000.0, size=n_samples)
    incomes[np.random.choice(n_samples, size=10, replace=False)] = np.nan
    data = pd.DataFrame({
        "age": ages,
        "income": incomes,
        "score": np.round(ages * 0.5 + np.random.normal(0, 5.0, size=n_samples), 1),
    })
    return data
...
def train_model():
    ...
    target_column = "score"
    X, y = build_features(df, feature_columns, target_column)
